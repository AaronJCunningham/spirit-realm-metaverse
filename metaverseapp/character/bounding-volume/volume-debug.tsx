...
import { BoundingVolume, Capsule } from "./use-bounding-volume";
...
function updateBox(
  box: THREE.Box3,
  boxRef: MutableRefObject<THREE.LineSegments>
) {
  const min = box.min;
  const max = box.max;
    ...
    <>
      <line2
        geometry={new LineGeometry().setPositions(
          createCapsulePoints(capsule.radius, capsule.height)
        )}
      >
      ...
      <line2
        ...
        geometry={new LineGeometry().setPositions(
          createCapsulePoints(capsule.radius, 0)
        )}
      >
        <lineMaterial color={color} linewidth={0.002} />
      </line2>
