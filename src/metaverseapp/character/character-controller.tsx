import { Stages, useUpdate, Vector3, useFrame } from "@react-three/fiber";
import {
  useCallback,
  useLayoutEffect,
  useRef,
...
import {
  CapsuleConfig,
  useBoundingVolume,
} from "./bounding-volume/use-bounding-volume";
...
export function CharacterController({
  ...
  useUpdate((_, delta) => {
    ...
    if (Math.abs(angleDelta) > Math.PI) {
      store.targetAngle =
        store.targetAngle - Math.sign(angleDelta) * Math.PI * 2;
    }

    store.currentAngle = smoothDamp.get(
      store.currentAngle,
      store.targetAngle,
      delta
    );
  ...
  const getIsGroundedMovement = useCallback(
    () => store.isGroundedMovement,
    [store]
  );
  ...
  function updateSocket() {
    if (socket !== undefined && meshRef.current !== null) {
      socket.emit("update", {
        ...
        qx: meshRef.current?.quaternion.x,
        qy: meshRef.current?.quaternion?.y,
  ...
  return (
    ...
    >
      <group position={position} ref={meshRef}>
        <group position={capsule === "auto" ? 0 : capsule.center}>
          {children}
        </group>
      </group>
