...
import { useCollider } from "./stores/collider-store";
import {
  acceleratedRaycast,
  computeBoundsTree,
  disposeBoundsTree,
...
export function Collider({
  ...
  const [bvhVisualizer, setBvhVisualizer] = useState<
    MeshBVHVisualizer | undefined
  >(undefined);
  ...
  const _debug = debug === false ? { collider: false, bvh: false } : debug;
  ...
  useUpdate(() => {
    ...
    if (
      Object.keys(store.boxMap).length !== Object.keys(store.prevBoxMap).length
    ) {
      rebuildBVH();
      store.prevBoxMap = { ...store.boxMap };
      return;
  ...
  return (
    <>
      ...
      {_debug && collider && <primitive visible={false} object={collider} />}
      {_debug && bvhVisualizer && (
        <primitive visible={false} object={bvhVisualizer} />
      )}
    </>
  );
}
